...
Course: ECE 4760
Purpose: Week 3 of Final Project
*/
    ...
    for(j = 0; j < h; j++){ //Traverse through rows
        for(i = 0; i < w; i++){ //Traverse through columns
            if(pgm_read_byte(bitmap + j * byteWidth + i / 8) & (128 >> (i & 7))) { //If the given is set
                ...
                tft_spiwrite16(0x0000); //Write the black color to the TFT. Change this if you want to use a different background color
            }
        }
    }
  _cs_high();
...
static PT_THREAD (protothread_buttons(struct pt *pt)){ //Buttons thread
    ...
    while(1){
        ...
        if(buttonID == 2 && buttonValue == 1 && myPlayer.y == 0 && startGame){ //If the button 2 was pressed and the game has started
            ...
            tft_fillRect(0, 0, WIDTH, HEIGHT, ILI9340_BLACK); //Clear the screen
    ...
    while(1){ //Infinite while loop
        ...
        if(startGame & myPlayer.alive){ //If the game has been started and the player is alive
            ...
            switch(obsType){ //Based on the value of the obstacle type
                case 0: drawBitmap(obstacle.x, (HEIGHT - GROUND_HEIGHT - ((obstacle.h / 2))), obsTypeZer, obstacle.w, obstacle.h, ILI9340_BLACK); //For small cactus
                        break;
                case 1: drawBitmap(obstacle.x, (HEIGHT - GROUND_HEIGHT - ((obstacle.h / 2))), obsTypeOne, obstacle.w, obstacle.h, ILI9340_BLACK); //For large cactus
                        break;
                case 2: drawBitmap(obstacle.x, (HEIGHT - GROUND_HEIGHT - ((obstacle.h / 2))), obsTypeTwo, obstacle.w, obstacle.h, ILI9340_BLACK); //For multi cactus
                        break;
            } 
            tft_fillRect(myPlayer.x, (HEIGHT - GROUND_HEIGHT - (myPlayer.y + (myPlayer.h / 2))), myPlayer.w, myPlayer.h, ILI9340_BLACK); //Clear the old display of the dino by drawing it with black 
            ...
            if(obstacle.x + obstacle.w < 0){ //If the obstacle is out of screen
                ...
                tft_setTextColor(ILI9340_BLACK); //Set the text color to black
                tft_setCursor(SCORE_X_OFFSET2, SCORE_Y_OFFSET1); //Set text coordinates
                sprintf(buffer, "%2d", score); //Send the score into the buffer array
                tft_writeString(buffer); //Print the text
                tft_setCursor(SCORE_X_OFFSET2, SCORE_Y_OFFSET2); //Set text coordinates
