...
Course: ECE 4760
Purpose: Week 2 of Final Project
*/
...
void __ISR(TIMER_2_VECTOR, ipl2) Timer2Handler(void){
...
static PT_THREAD (pt_anim(struct pt *pt)){ //Animation protothread
    ...
    while(1){ //Infinite while loop
        ...
        if(startGame & myPlayer.alive){ //If the game has been started and the player is alive
            ...
            tft_fillRect(obstacle.x, (HEIGHT - GROUND_HEIGHT - ((obstacle.h / 2))), obstacle.w, obstacle.h, ILI9340_BLACK); //Clear the old display of the obstacle by drawing it with black 
            ...
            tft_fillRect(obstacle.x, (HEIGHT - GROUND_HEIGHT - ((obstacle.h / 2))), obstacle.w, obstacle.h, HARD_COLOR); //Draw the obstacle at updated position
            for(i = 0; i < GROUND_SIZE; i++){ //For all elements ranging in the ground array
                tft_drawFastHLine(ground[i].x, ground[i].y, ground[i].w, HARD_COLOR); //Draw the new display of the ground elements
            }
            tft_fillRect(myPlayer.x, (HEIGHT - GROUND_HEIGHT - (myPlayer.y + (myPlayer.h / 2))), myPlayer.w, myPlayer.h, SOFT_COLOR); //Draw the dino at updated position
